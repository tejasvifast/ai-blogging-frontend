...
export async function generateMetadata({
  ...
  const category = await publicApi.getCategory(slug).catch(() => null);
...
export default async function CategoryPage({
  ...
  const listing = await publicApi
    .listPosts({ category: slug, page, limit: DEFAULT_PAGE_SIZE, sort })
    ...
    <Container className="py-10 sm:py-14">
      ...
      <header className="mb-8 mt-6 flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <h1 className="mt-1 font-serif text-4xl font-bold sm:text-5xl">{category.name}</h1>
